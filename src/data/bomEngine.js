...
export const PROJECT_TEMPLATES = [
  {
    ...
    fields: [
      ...
      {
        key: "saklar_ganda",
        label: "Pakai saklar 2 gang?",
        type: "bool",
        default: false,
      },
      {
        key: "premium",
        label: "Material premium (Schneider/Philips)",
        type: "bool",
        default: false,
      },
    ],
  },
  {
    ...
    fields: [
      ...
      {
        key: "outdoor",
        label: "Ada outdoor / waterproof?",
        type: "bool",
        default: false,
      },
      {
        key: "outdoor_qty",
        label: "Qty outdoor (jika ada)",
        type: "number",
        default: 2,
        min: 0,
        max: 20,
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: "amp",
        label: "Ampere MCB utama",
        type: "select",
        options: ["6A", "10A", "16A", "20A", "25A"],
        default: "20A",
      },
      {
        key: "premium",
        label: "MCB Schneider (premium)",
        type: "bool",
        default: true,
      },
    ],
    ...
    fields: [
      {
        ...
        type: "textarea",
        default: "Pasang 10 lampu dan 6 stop kontak di rumah 2 lantai",
      },
      {
        key: "premium",
        label: "Material premium",
...
const SKU_PREFS = {
  ...
  kabel_nym_25: {
    premium: ["KBL-NYM-25"],
    budget: ["KBL-NYM-25", "KBL-SRB-15"],
  },
  kabel_nym_325: {
    premium: ["KBL-NYM-325"],
    budget: ["KBL-NYM-325", "KBL-NYM-25"],
  },
...
function parseCustomPrompt(text) {
  ...
  return {
    lampu:
      num(/(?:pasang\s+)?(\d+)\s*(?:titik\s+)?lampu/) ||
      num(/lampu\s*(\d+)/) ||
      (t.includes("lampu") ? 8 : 0),
    stop:
      num(/(\d+)\s*stop\s*kontak/) ||
      num(/stop\s*kontak\s*(\d+)/) ||
      (t.includes("stop") ? 4 : 0),
    saklar: num(/(\d+)\s*saklar/) || (t.includes("saklar") ? 4 : 0),
    lantai:
      num(/(\d+)\s*lantai/) ||
      (t.includes("2 lantai") || t.includes("dua lantai") ? 2 : 1),
...
export function calculateBOM(templateId, params, products) {
  ...
  if (templateId === "lampu_rumah") {
    ...
    add(
      "kabel_nym_25",
      meterKabel,
      `${titik} titik × ~8m + riser ${lantai} lantai`,
      1.15,
    );
  ...
  if (templateId === "custom") {
    ...
    if (!items.length) {
      warnings.push(
        "Prompt kurang jelas — coba sebutkan jumlah lampu/stop kontak",
      );
      // minimal fallback
  ...
  for (const it of items) {
    ...
    if (merged[it.sku]) {
      ...
      merged[it.sku].shortage = merged[it.sku].stock_ok
        ? 0
        : merged[it.sku].qty - merged[it.sku].stock;
    } else {
      merged[it.sku] = { ...it };
    }
  }
