...
export default function StockTransaction({ mode = "in" }) {
  ...
  const findCatFromCode = (code) => {
    const q = String(code || "")
      .trim()
      .toUpperCase();
    const entry = Object.entries(categoryBarcodes).find(
      ([, v]) => v === q || v.toLowerCase() === q.toLowerCase(),
    );
  ...
  const submit = async () => {
    ...
    if (isIn) {
      if (!String(supplier || "").trim())
        return show("err", "Pilih supplier dari Master Supplier");
      if (!suppliers.some((s) => s.name === supplier))
        return show("err", "Supplier harus ada di Master Supplier");
    } else {
      if (!String(customer || "").trim())
        return show("err", "Isi nama pelanggan dulu");
    }
    try {
      ...
      show(
        "ok",
        `${isIn ? "Barang masuk" : "Barang keluar"} tersimpan: ${selected.name} (${q})`,
      );
      setQty(1);
      setNotes("");
      if (!isIn) setCustomer("");
  ...
  const applyPOItem = async (po, item) => {
    const product = products.find(
      (p) => String(p.id) === String(item.product_id),
    );
    if (!product)
      return show("err", `Produk ${item.name} tidak ditemukan di master`);
    try {
      ...
      show("ok", `${po.no}: +${item.qty} ${item.name} dari ${po.supplier}`);
      // jika semua item PO sudah bisa dianggap received, tandai
      updatePOStatus?.(po.id, "received");
      setHistoryLocal((prev) =>
        [
          {
            id: `po-${po.id}-${item.product_id}`,
            product_name: item.name,
            change: Number(item.qty) || 1,
            stock_before: product.stock,
            stock_after: product.stock + (Number(item.qty) || 1),
            supplier: po.supplier,
          },
          ...prev,
        ].slice(0, 20),
      ...
      show("err", e.message || "Gagal terapkan PO");
    }
  };
  ...
  const startCamera = async () => {
    if (!("BarcodeDetector" in window)) {
      show(
        "err",
        "Kamera barcode belum didukung browser. Gunakan scanner USB / ketik manual.",
      );
      return;
    }
    try {
      ...
      show("err", "Gagal akses kamera. Izinkan kamera.");
      stopCamera();
    }
  };
  ...
  return (
    <div>
      ...
      <div className="page-subtitle">
        {isIn
          ? "Input restock manual atau scan barcode"
          : "Catat barang keluar manual atau scan barcode"}
      ...
      {msg && (
        <div
          style={{
            ...
            background:
              msg.type === "ok" ? "rgba(5,150,105,0.1)" : "rgba(220,38,38,0.1)",
      ...
      >
          ...
          >
            <div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                >
                  <i
                    className={`fas ${cameraOn ? "fa-times" : "fa-camera"}`}
                  ></i>
                </button>
              </div>
              <div style={{ fontSize: 11, color: "#64748B", marginTop: 4 }}>
                Scanner USB otomatis + Enter. Kode kategori: MCB=CAT-MCB,
                Fitting=CAT-FITTING, Kabel=CAT-KABEL, Stop
                Kontak=CAT-STOPKONTAK, Steker=CAT-STEKER, Saklar=CAT-SAKLAR,
                Panel=CAT-PANEL, Lampu=CAT-LAMPU
              </div>
            </div>
                      ...
                      style={{
                        ...
                        border:
                          category === c
                            ...
                            : "1px solid #E6E8EA",
                        background:
                          category === c ? "rgba(0,86,179,0.09)" : "#fff",
                        color: category === c ? "#004494" : "#475569",
                      }}
                    >
                      {c}
                    </button>
            ...
            <div>
              <label style={s.label}>
                Produk {category && category !== "Semua" ? `· ${category}` : ""}{" "}
                ({filtered.length})
              </label>
              <select
                value={selectedId}
            ...
            {selected && (
              ...
              >
                ...
                <div>
                  ...
                  <br />
                  <strong>{selected.min_stock ?? "-"}</strong>
                </div>
                <div>
                  <span style={{ color: "#64748B" }}>SKU</span>
                  <br />
                  <strong>{selected.sku || selected.id}</strong>
                </div>
                <div>
            ...
            {isIn ? (
              <div>
                ...
                {!suppliers.length && (
                  <div style={{ fontSize: 11, color: "#DC2626", marginTop: 4 }}>
                    Master Supplier kosong. Isi dulu di menu Master Supplier.
                  </div>
                )}
                <div style={{ fontSize: 11, color: "#64748B", marginTop: 4 }}>
                  Wajib dari Master Supplier — terhubung ke EOQ (lead time &
                  biaya pesan).
                </div>
              </div>
            ) : (
              <div>
                <label style={s.label}>
                  Pelanggan <span style={{ color: "#DC2626" }}>*</span>
                </label>
                <input
                ...
                <div style={{ fontSize: 11, color: "#64748B", marginTop: 4 }}>
                  Isi bebas atau pilih saran. Data keluar tetap jadi input
                  Apriori. Penagihan via menu Keuangan.
                </div>
              </div>
            )}
            ...
            <button
              type="button"
              onClick={submit}
              style={{ ...s.btnP, height: 44, fontSize: 14 }}
            >
              <i
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div className="panel">
            <div className="panel-head">
              <h3>Barcode per Kategori</h3>
              <span className="badge-pill">BISA</span>
            </div>
          ...
          {isIn && (
            <div className="panel">
              <div className="panel-head">
                <h3>Terapkan Draft PO</h3>
                <span className="badge-pill">{openPOs.length} open</span>
              </div>
              <div className="panel-body" style={{ paddingTop: 8 }}>
                {!openPOs.length ? (
                  ...
                  >
                    Tidak ada Draft PO. Buat dulu di menu{" "}
                    <b>Rekomendasi Pengadaan</b>.
                  </div>
                ) : (
                  <div
          ...
          <div className="panel" style={{ flex: 1 }}>
            ...
            <div className="panel-body" style={{ paddingTop: 8 }}>
              {historyLocal.length === 0 ? (
                <div
                  style={{ color: "#64748B", fontSize: 13, padding: "12px 0" }}
                >
                  Belum ada transaksi sesi ini.
                </div>
              ) : (
                <div className="activity-list">
                  {historyLocal.map((h) => (
                    <div key={h.id} className="activity-row">
                      <div
                        className={`activity-icon ${h.change > 0 ? "in" : "out"}`}
                      >
                        <i
                          className={`fas ${h.change > 0 ? "fa-arrow-down" : "fa-arrow-up"}`}
                        ></i>
                      </div>
