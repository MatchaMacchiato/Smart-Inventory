...
export default function BomAI({ onNavigate }) {
  ...
  const template =
    PROJECT_TEMPLATES.find((t) => t.id === templateId) || PROJECT_TEMPLATES[0];
  ...
  const exportCSV = () => {
    if (!result) return;
    const header =
      "SKU,Nama,Kategori,Qty,Unit,Harga,Subtotal,Stok,Status,Catatan";
  ...
  const toInvoice = async () => {
    ...
    try {
      ...
      setToast({
        type: "ok",
        msg: `Draft faktur ${formatRp(nilai)} dibuat untuk ${nama}`,
      });
    } catch {
      setToast({
        type: "warn",
        msg: `Faktur lokal siap: ${formatRp(nilai)} (server off — buka Keuangan untuk input manual)`,
      });
    }
  ...
  const shortageLines = useMemo(
    () => (result?.lines || []).filter((r) => !r.stock_ok),
    [result],
  );
  ...
  return (
    <div>
      ...
      <div className="page-subtitle">
        Ketik spek pekerjaan → AI hitung material + cek stok + estimasi biaya.
        Anti-mainstream buat toko listrik.
      </div>
          ...
          style={{
            ...
            background:
              toast.type === "ok"
                ? "rgba(5,150,105,0.1)"
                : "rgba(255,115,0,0.12)",
            color: toast.type === "ok" ? "#059669" : "#FF7300",
          }}
        >
          {toast.msg}
        ...
        {PROJECT_TEMPLATES.map((t) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              border:
                templateId === t.id ? "2px solid #0056b3" : "1px solid #E6E8EA",
              background: templateId === t.id ? "rgba(0,86,179,0.09)" : "#fff",
            }}
          >
      ...
      >
        ...
        <div className="panel">
          <div className="panel-head">
            <h3>Parameter · {template.label}</h3>
          </div>
          <div
          ...
          >
            ...
            <label style={{ fontSize: 12, fontWeight: 700, color: "#94A3B8" }}>
              ...
              <input
                ...
                placeholder="cth. Pak Budi / AMIN ELECTRIC"
                style={{ ...sel, marginTop: 6 }}
              />
            </label>
            ...
            >
              {busy ? (
                ...
                <>
                  <i className="fas fa-magic" style={{ marginRight: 8 }}></i>
                  Hitung Material
                </>
              )}
            </button>
        ...
        <div className="panel">
          <div className="panel-head">
            <h3>Hasil BOM</h3>
            {result && (
              <span className="badge-pill">{result.lines.length} item</span>
            )}
          </div>
            ...
            {result && (
              <>
                  ...
                  >
                    <thead>
                      <tr style={{ background: "#F1F5F9" }}>
                        {["Produk", "Qty", "Harga", "Subtotal", "Stok"].map(
                          (h) => (
                            <th
                              key={h}
                              style={{
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ fontWeight: 800, fontSize: 16 }}>
                      {formatRp(result.total_material)}
                    </div>
                  </div>
                  <div
                    style={{
                ...
                {shortageLines.length > 0 && (
                  ...
                  >
                    <b>⚠ {shortageLines.length} item stok kurang</b> — sarankan
                    restock dulu atau ganti ke brand lain.
                    <div style={{ marginTop: 6 }}>
                      {shortageLines
                        .map((r) => `${r.sku} (−${r.shortage})`)
                        .join(" · ")}
                    </div>
                  </div>
                )}
