...
const SUGGESTIONS = [
  {
    id: "predictions",
    title: "🔮 Prediksi Stok AI",
    desc: "Perkiraan kapan stok habis + saran restock per produk",
    icon: "fa-chart-line",
    color: "#0056b3",
  },
  {
    ...
    desc: "Lihat produk di bawah min stok + rekomendasi restock",
  ...
  },
  {
    ...
    desc: "Prioritas barang yang harus dibeli",
    icon: "fa-cart-plus",
    color: "#059669",
  },
  {
    id: "movement-summary",
    title: "Ringkas masuk/keluar",
    desc: "Total pergerakan stok + analitik",
    icon: "fa-exchange-alt",
    color: "#0056b3",
  },
  {
    id: "category-health",
    title: "Kesehatan per kategori",
    desc: "Kategori rawan habis berdasarkan data real",
    icon: "fa-chart-pie",
    color: "#7C3AED",
  },
  {
    ...
    desc: "Ringkasan semua aspek stok toko",
...
export default function AIAsisten({ onNavigate }) {
  ...
  const localPredictions = useMemo(() => {
    ...
    return products
      .map((p) => {
        ...
        const daysLeft =
          avgDaily > 0
            ? Math.round((stock / avgDaily) * 10) / 10
            : stock <= min
              ? 0
              : null;
        let urgency = "aman";
        if (stock <= 0) urgency = "habis";
        else if (stock <= min || (daysLeft !== null && daysLeft <= 3))
          urgency = "kritis";
        ...
        return {
          ...
          confidence:
            totalOut > 0 ? (totalOut >= 10 ? "tinggi" : "sedang") : "rendah",
        };
      })
      .sort((a, b) => {
        const order = { habis: 0, kritis: 1, waspada: 2, pantau: 3, aman: 4 };
        return (
          (order[a.urgency] - order[b.urgency]) * 1000 +
  ...
  const run = (id) => {
    ...
    setTimeout(() => {
      try {
        if (id === "predictions") {
          const critical = predRows.filter((r) =>
            ["habis", "kritis", "waspada"].includes(r.urgency),
          );
          const totalSuggest = predRows.reduce(
            (s, r) => s + (r.suggest_restock || 0),
            0,
          );
          setResult({
            type: "predictions",
            ...
            metrics: [
              {
                label: "Kritis/Habis",
                value: predRows.filter(
                  (r) => r.urgency === "kritis" || r.urgency === "habis",
                ).length,
                color: "#DC2626",
              },
              {
                label: "Waspada",
                value: predRows.filter((r) => r.urgency === "waspada").length,
                color: "#FF7300",
              },
              {
                label: "Pantau",
                value: predRows.filter((r) => r.urgency === "pantau").length,
                color: "#0EA5E9",
              },
              {
                label: "Aman",
                value: predRows.filter((r) => r.urgency === "aman").length,
                color: "#059669",
              },
        ...
        if (id === "low-stock" || id === "restock-plan") {
          ...
          setResult({
            type: id,
            title:
              id === "restock-plan" ? "Rencana Restock" : "Produk Stok Menipis",
            summary:
              id === "restock-plan"
            ...
            rows: rows.length
              ? rows
              : [{ name: "Semua stok aman", stock: "-", min_stock: "-" }],
            actions: [
              { label: "Barang Masuk", go: "barang-masuk" },
              { label: "Data Produk", go: "produk" },
            ],
        ...
        if (id === "category-health") {
          ...
          const cats = Object.values(catMap)
            .map((c) => ({
              ...
              health:
                c.total_stock >= c.total_min
                  ? "Aman"
                  : c.total_stock >= c.total_min * 0.5
                    ? "Perlu pantau"
                    : "Rawan",
            }))
            .sort(
              (a, b) =>
                a.total_stock / Math.max(1, a.count) -
                b.total_stock / Math.max(1, b.count),
            );
          setResult({
        ...
        if (id === "overview") {
          setResult({
            ...
            metrics: [
              { label: "Produk", value: products.length, color: "#0056b3" },
              {
                label: "Kategori",
              ...
              {
                label: "Total Stok",
                value: Number(stats.total_stock || 0).toLocaleString(),
                color: "#059669",
              },
              {
                label: "Stok Menipis",
              ...
              { label: "Transaksi", value: history.length, color: "#FF7300" },
            ],
            actions: [
  ...
  const freeTextHint = useMemo(() => {
    ...
    if (q.includes("kategori") || q.includes("sehat") || q.includes("rawan"))
      return "category-health";
    if (q.includes("ikhtisar") || q.includes("overview") || q.includes("semua"))
      return "overview";
    return null;
  }, [chat]);
  ...
  return (
    <div>
      ...
      <div className="page-subtitle">
        Rekomendasi stok berbasis data nyata &middot;{" "}
        <strong>terhubung ke update produk & transaksi</strong>
      </div>
      ...
      <div className="panel" style={{ marginBottom: 16 }}>
        <div className="panel-head">
          <h3>Tanya cepat</h3>
          <span className="badge-pill">RULE-BASED AI</span>
        </div>
        <div className="panel-body" style={{ display: "flex", gap: 8 }}>
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && freeTextHint) run(freeTextHint);
            }}
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              borderRadius: 10,
              background: freeTextHint ? "#0056b3" : "#E6E8EA",
              color: freeTextHint ? "#fff" : "#64748B",
              fontWeight: 700,
              cursor: freeTextHint ? "pointer" : "not-allowed",
            }}
          >
            Analisis
          </button>
      ...
      <div className="panel">
        <div className="panel-head">
          <h3>{result?.title || "Hasil Asisten"}</h3>
          {loading && <span className="badge-pill">Memproses...</span>}
        </div>
        <div className="panel-body">
          {!result && !loading && (
            <div style={{ color: "#64748B", fontSize: 13 }}>
              Pilih salah satu kartu di atas, atau ketik pertanyaan.
            </div>
          )}
          {loading && (
            <div style={{ color: "#0056b3", fontWeight: 600, fontSize: 13 }}>
              <i
              ...
              {result.rows?.length > 0 && result.type === "predictions" && (
                ...
                >
                  ...
                  <tbody>
                    {result.rows.map((r, i) => {
                      const color =
                        r.urgency === "habis" || r.urgency === "kritis"
                          ? "#DC2626"
                          : r.urgency === "waspada"
                            ? "#FF7300"
                            : r.urgency === "pantau"
