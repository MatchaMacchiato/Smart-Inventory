    ...
    .map((p) => {
      ...
      if (avgDaily <= 0) {
        return {
          ...
          status: "no_demand",
          recommendation: `Tidak ada penjualan ${days} hari — jangan restock`,
        };
      }
...
function computeLocalApriori(history, params) {
  ...
  const nameOf = (id) =>
    rows.find((r) => r.product_id === id)?.product_name || `Produk #${id}`;
  ...
  return {
    ...
    message: rules.length
      ? null
      : "Data lokal kurang — turunkan threshold atau input barang keluar dulu.",
  };
}
    ...
    <div className="panel" style={{ marginBottom: 10 }}>
      <div className="panel-body" style={{ padding: 14 }}>
        ...
        >
          ...
          <span style={{ fontWeight: 900, color: "#0056b3", fontSize: 15 }}>
            →
          </span>
          <span style={{ fontWeight: 800, fontSize: 13, color: "#004494" }}>
        ...
        >
          <span>
            <b style={{ color: "#002a5c" }}>{pct(rule.confidence)}</b>{" "}
            confidence
          </span>
          <span>
            <b style={{ color: "#002a5c" }}>{pct(rule.support)}</b> support
          </span>
          <span>
            Lift{" "}
            <b style={{ color: rule.lift >= 1 ? "#059669" : "#DC2626" }}>
...
export default function AIIntelligence() {
  ...
  const runEoq = () => {
    ...
    aiIntelApi
      ...
      .catch((e) => {
        ...
        setSourceNote("lokal (API gagal)");
        setError(
          `API: ${errText(e, "gagal")} — pakai hitungan lokal dari history frontend`,
        );
      })
      .finally(() => setLoading(false));
  };
  ...
  const statusMeta = {
    ...
    watch: { label: "WASPADA", color: "#FF7300", bg: "rgba(255,115,0,0.12)" },
  ...
  return (
    <div>
      ...
      <div className="page-subtitle">
        Analisis data penjualan deterministik — bundling (Apriori) & optimasi
        restock (EOQ).
        {sourceNote && (
          <span
            style={{
              marginLeft: 8,
              color: sourceNote.includes("lokal") ? "#FF7300" : "#059669",
              fontWeight: 700,
            }}
          >
            · sumber: {sourceNote}
      ...
      >
        {[
          {
            id: "bundling",
            label: "🛒 Bundling (Apriori)",
            desc: "Produk yang sering dibeli bareng",
          },
          {
            id: "eoq",
            label: "📦 Optimasi Restock (EOQ)",
            desc: "Qty pesanan optimal + reorder point",
          },
        ].map((t) => (
          <button
            key={t.id}
            ...
            style={{
              padding: "10px 16px",
              borderRadius: 12,
              border: tab === t.id ? "2px solid #0056b3" : "1px solid #E6E8EA",
              background: tab === t.id ? "rgba(0,86,179,0.09)" : "#fff",
              cursor: "pointer",
              textAlign: "left",
            }}
          >
            ...
            >
              ...
              >
                ...
                <select
                  ...
                  style={{ ...sel, width: "100%", marginTop: 6 }}
                >
                  {[30, 60, 90, 180].map((d) => (
            ...
            <div className="panel">
              ...
              <div className="panel-body" style={{ padding: 14 }}>
                {!apriori?.rules?.length && (
                  ...
                  >
                    Belum ada rule. Turunkan threshold atau pastikan ada data
                    barang keluar.
                  </div>
                )}
                {apriori?.rules?.map((r, i) => (
          ...
          <div className="panel">
            ...
            >
              ...
              >
                ...
                <select
                  value={eoqDays}
                  onChange={(e) => setEoqDays(Number(e.target.value))}
                  style={{ ...sel, width: "100%", marginTop: 6 }}
                >
                  {[30, 60, 90, 180].map((d) => (
                    <option key={d} value={d}>
          ...
          <div>
            ...
            {!eoq && !loading && (
              <div className="panel">
                ...
                >
                  Klik Hitung Ulang untuk hitung EOQ
                </div>
              </div>
            )}
          </div>
